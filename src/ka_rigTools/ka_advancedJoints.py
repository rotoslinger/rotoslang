...
def ka_constraints.localizeConstraint(constraint):
...
import transformHelper as trh#;reload(trh)


expression = """import pymel.core as pymel

thisnode = pymel.ls(thisnode)[0]
thisControl = thisnode.inputX.inputs()[0]
selection = pymel.ls(selection=True)
if thisControl in selection:
    inputValue = x
    nextControl = thisnode.nextPointControl.inputs()[0]


    else:
       #nextControl.inheritsTransform.set(0)
       if len(selection) == 1:
           pymel.select(nextControl)

    if not thisControl.t.inputs() and not thisControl.r.inputs():
	transformValue = pymel.xform(thisControl, query=True, translation=True, worldSpace=True)
	if round(inputValue, 2):
	    transformValue[0] += 1
	    transformValue[1] += 1
	    transformValue[2] += 1

	pymel.xform(nextControl, translation=[transformValue[0], transformValue[1], transformValue[2]], worldSpace=True)

	transformValue = pymel.xform(thisControl, query=True, rotation=True, worldSpace=True)
	pymel.xform(nextControl, rotation=transformValue, worldSpace=True)
"""
