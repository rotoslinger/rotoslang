...
#def _clearAttrDataBase():
    #global attrDataBase
    #attrDataBase = {}
    #pickle.dump( attrDataBase, open( attrDataBaseFile, "wb" ) )

#attrDataBaseFile = os.path.join(os.path.dirname(__file__), 'attrDataBase.p')
#attrDataBase = _getattrDataBase()


#import pymel.core as pymel
#import ka_rigTools.ka_attrTool.attributeObj as attributeObj ; reload(attributeObj)
#if not pymel.objExists('pCubeShape1'):
    #node = pymel.polyCube()[1]
#else:
    #node = pymel.ls('pCubeShape1')[0]

#import ka_rigTools.ka_attrTool.attributeObj as attributeObj ; reload(attributeObj)
#attrObj = attributeObj.AttributeObj(node, 'colorPerVertex')
#vertexColor
...
#attrObj.attrName()
#attrObj.attr()
#attrObj.isUserDefined()
#attrObj.nodeName()
#attrObj.nodeLongName()
#attrObj.attrShortName()
#attrObj.attrLongName()
#attrObj.attrParent()
#attrObj.attrParentName()
#attrObj.attrParentLongName()
#attrObj.attrChildren()
#attrObj.attrChildrenNames()
#attrObj.isArray()
#attrObj.isElement()
#attrObj.isCompound()
#attrObj.isMulti()
...
#import ka_rigTools.ka_attrTool.attributeObj as attributeObj ; reload(attributeObj)
#attrObj = attributeObj.AttributeObj(node, 'input1X')
...
#attrObj.isWritable()
#attrObj.value()
#attrObj.isDefaultValue()
#attrObj.isConnected()
#attrObj.inputs()
#attrObj.outputs()
#attrObj.isLocked()
#attrObj.isKeyable()
#attrObj.isInChannelBox()
#attrObj.isSimple()
#attrObj.attrType()
...
class AttributeObj(object):
    ...
    def __init__(self, node, attrName, **kwargs):
        ...
        self._isAttributeType        = kwargs.get('_isAttributeType', None)
        self._enumStrings            = kwargs.get('_enumStrings', None)
        self._enumString             = kwargs.get('_enumString', None)
        self._hasMinValue            = kwargs.get('_hasMinValue', None)
        self._hasMaxValue            = kwargs.get('_hasMaxValue', None)
        self._minValue               = kwargs.get('_minValue', None)
        self._maxValue               = kwargs.get('_maxValue', None)
        self._hasSoftMaxValue        = kwargs.get('_hasSoftMaxValue', None)
        self._hasSoftMinValue        = kwargs.get('_hasSoftMinValue', None)
        self._softMaxValue           = kwargs.get('_softMaxValue', None)
        self._softMinValue           = kwargs.get('_softMinValue', None)
    ...
    def __repr__(self):
        return '<AttributeObject: %s.%s>' % (self.nodeName(), self._attrName)

    def _getFromDataBase(self, propertyName):
        if not self._isUserDefined:

            nodeTypeDict = self._nodeTypeDBInfo
            attrDict = nodeTypeDict.get(self.attrLongName(), {})
    ...
    def _addToDataBase(self, propertyName, propertyValue):
        if not self.isUserDefined():
            ...
            #self._nodeTypeDBInfo = _getattrDataBase(nodeType)
            attrLongName = self.attrLongName()

            if attrLongName not in self._nodeTypeDBInfo:
    ...
    def nodeLongName(self, refresh=False):
        ...
        try:
            return self._node.name(int=True)
    ...
    def value(self, refresh=False):
        ...
        #try:
        if self.exists():
            #if not self.isMulti() and not self.isArray() and not self.isCompound():
            if not self.isMulti() and not self.isArray() and not self.isCompound():
                value = pymel.getAttr(self.attr())
                if isinstance(value, float) or isinstance(value, str) or isinstance(value, bool) or isinstance(value, int):
