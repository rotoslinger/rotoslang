...
//                    MDoubleArray nOutWeight;
    ...
    //                        std::vector < double > tempUPointVals;
                            ...
                            for (; !iter.isDone();)
                            {
                                ...
                                double tmpW;
                                if (allWeightsArray.size() > iIndex and allUAnimCurveWeights.size() > iIndex and allVAnimCurveWeights.size() > iIndex)
...
//                MGlobal::displayInfo(MString("WORKING")+myfloat);
    ...
    //                    CheckStatusReturn( status, "Unable to get outputs" );
                        ...
                        status = child.setMObject(oWeights);child.setClean();
...
//                        child.setClean();
                        ...
                        /*
                        MDataHandle child(handle.child( allOutWeightChildren[j] ) );
//                        MObject mDoubleArrayData = handle.child( allOutWeightChildren[j] ).data;

                        MObject mDoubleArrayData = child.data();
//                        MFnDoubleArrayData newData(child.builder(), &status);
                        MFnDoubleArrayData newData(mDoubleArrayData, &status);
...
MStatus LHWeightDeformer::initialize() {
    ...
    dArrayDataFn.create(defaultDoubleArray);



    //---child weight
    aWeights = tAttr.create("pointWeight", "pweight", MFnData::kDoubleArray, dArrayDataFn.object());
...
//    tAttr.setInternal(true);
    ...
    //---child weight
    aOutWeights = tAttr.create("outWeights", "ow", MFnData::kDoubleArray, dArrayDataFn.object());
    tAttr.setArray(false);
    tAttr.setReadable(true);
    tAttr.setWritable(false);
    tAttr.setKeyable(false);
