...
MString locatorTemplate::drawDbClassification("drawdb/geometry/locatorTemplate");

MString locatorTemplate::drawRegistrantId("locatorTemplatePlugin");

locatorTemplate::locatorTemplate() {}

locatorTemplate::~locatorTemplate() {}

void* locatorTemplate::creator() {
  return new locatorTemplate();
}
bool locatorTemplate::isBounded() const
{
    return true;
}
MBoundingBox locatorTemplate::boundingBox() const
{
...
void locatorTemplate::draw(M3dView &view, const MDagPath &path,
                              ...
                              M3dView::DisplayStatus status)
{
    ...
    MPlug matrixPlug(thisNode, locatorTemplate::worldMatrix);
...
locatorTemplateOverride::locatorTemplateOverride(const MObject& obj)
    : MPxDrawOverride(obj, NULL) {}

locatorTemplateOverride::~locatorTemplateOverride() {}

MHWRender::DrawAPI locatorTemplateOverride::supportedDrawAPIs() const {
    return (MHWRender::kOpenGL | MHWRender::kDirectX11 | MHWRender::kOpenGLCoreProfile);
}

bool locatorTemplateOverride::isBounded(const MDagPath&, const MDagPath&) const {
  return true;
}

MBoundingBox locatorTemplateOverride::boundingBox(const MDagPath& objPath, const MDagPath& cameraPath) const {
    static MBoundingBox circleBBox;
    for (int i = 0; i < circleCount; i++)
    {
    MPoint shapePoint(circle[i][0], circle[i][1], circle[i][2]);
...
LocatorCapsuleData locatorTemplateOverride::getPlugValuesFromLocatorNode(const MDagPath &objPath) const
{
    ...
    MPlug plug(locatorNode, locatorTemplate::aSize);
    double sizeVal;
    plug.getValue(sizeVal);
    rData.size = sizeVal;

    //Get Locators World Matrix
    MPlug matrixPlug(locatorNode, locatorTemplate::worldMatrix);
    MFnMatrixData matrixData(matrixPlug.asMObject());
    MMatrix worldMatrix = matrixData.matrix();
...
MUserData* locatorTemplateOverride::prepareForDraw(const MDagPath& objPath, const MDagPath& cameraPath,
...
void locatorTemplateOverride::addUIDrawables(const MDagPath& objPath,
...
MStatus locatorTemplate::initialize() {
  MStatus status;
