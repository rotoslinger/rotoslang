...
// baseMeshAttrs
MObject LHVectorDeformer::aBaseMesh;
MObject LHVectorDeformer::aBaseMeshParent;
MObject LHVectorDeformer::aUseBaseMesh;
...
//    for( index = 0; index < indicesLength; ++index)
    ...
    int useBaseMesh = data.inputValue(aUseBaseMesh).asInt();
...
//        MGlobal::displayInfo(MString()+myfloat);
    ...
    if (useBaseMesh == 1)
    {
        geomArrayHandle = data.inputArrayValue(aBaseMeshParent);
    }
//    MArrayDataHandle geomBaseArrayHandle = data.inputArrayValue(aBaseMeshParent);
    // cache weight Mesh----
...
MStatus LHVectorDeformer::initialize() {
    ...
    // cache attributes
    aUseBaseMesh = nAttr.create("useBaseMesh", "uBaseMesh", MFnNumericData::kInt);
    nAttr.setKeyable(true);
    nAttr.setMin(0.0);
    nAttr.setMax(1.0);
    nAttr.setDefault(1.0);
    addAttribute(aUseBaseMesh);
...
//
    ...
    attributeAffects(aUseBaseMesh, outputGeom);
