...
//    for( index = 0; index < indicesLength; ++index)
    ...
    // typed attrs
    MObject oCurve = data.inputValue(aOutCurve).asNurbsCurveTransformed();
            ...
            if (success == 1)
            {
                ...
                if (iterGeoCount > 0)
                {
                    for (; !iter.isDone();)
                    {
                	    pt = iter.position();
                	    idx = iter.index();
                	    tmpPoint[idx] = fnCurve.closestPoint( pt, &tmpParam[idx], 0.00001, MSpace::kObject);
                        iter.next();

                    }
                    iter.reset();
                    closestPoint.push_back(tmpPoint);
                    closestParam.push_back(tmpParam);
                }
                else
                {
        ...
        for(i = 0; i < numIndex; i++ )
        {
            ...
            if (success == 1)
            {
                ...
                if (iterGeoCount > 0)
                {
                    ...
                    {
                	    idx = iter.index();
                	    tmpTangent[idx] = fnCurve.tangent( closestParam[i][idx], MSpace::kObject);
                        iter.next();

                    }
                    iter.reset();
                    closestTangent.push_back(tmpTangent);
                }
                else
                {
                	MVector tmp;
...
//        pt = pt * rW;
        ...
        {
        	if (closestTangent.size() > mIndex)
        	{
            	if (closestTangent[mIndex].length() > idx)
            	{
            		// convert from radians
            		double degree = (allVals[0][j] * (3.14159265/180.0 ));
					//Compose rotation
					MQuaternion rotate(degree,closestTangent[mIndex][idx]);
					MMatrix rotateMatrix = rotate.asMatrix();

					//Snap point to pivot
					MVector toCenterBase(-(closestPoint[mIndex][idx].x),
										 -(closestPoint[mIndex][idx].y),
										 -(closestPoint[mIndex][idx].z));
					pt = pt + toCenterBase;
