...
//    MObject LHWeightDeformer::aOutWeightsParentArray;
    ...
    if (allOutWeightsArray.size() < numIndex or cacheOutWeightsAmt == 0)
    {
        ...
        std::vector < std::vector < MDoubleArray > > tmpAllWeightsArray;
//        std::vector < std::vector < std::vector < double > > > tmpAllSumWeightsArray;
        for(i = 0; i < numIndex; i++ )
        {
            //3-d arrays
            std::vector < MDoubleArray > tempWeights;
            ...
            if (success == 1)
            {
                ...
                if (iterGeoCount > 0)
                {
                    for (; !iter.isDone();)
                    {
                        ...
                        //////////////////////////////////////////////////////////////////////////////////
//                    MDoubleArray tempSumOutWeights;
//                    std::vector < std::vector < double > > allPaintWeights;
                    std::vector < std::vector < double > > tempSumOutWeights;
                    for(j = 0; j < allWeightsArray[i].size(); j++ )
                    {
                        std::vector < double > tmpSumWeights;

                        for(k = 0; k < allWeightsArray[i][j].size(); k++ )
                        {

                            if (allWeightsArray.size() > i)
                            {
                                if (allWeightsArray[i].size() > j)
                                {
                                    if (allWeightsArray[i][j].size() > j)
                                    {
                                        if (allWeightsArray[i][j][k].length() > idx)
                                        {
                                            tempW = allWeightsArray[mIndex][j][k][idx]*
                                              allUAnimCurveWeights[mIndex][j][k][idx] *
                                              allVAnimCurveWeights[mIndex][j][k][idx];
...
//                            allW = std::accumulate(tempSumOutWeights[0].begin(),tempSumOutWeights[0].end(),0.0);
//                            tmpOutWeights.append(allW);
                            tmpSumWeights.push_back(tempW);
                        }
//                        allW = std::accumulate(tempSumOutWeights[0].begin(),tempSumOutWeights[0].end(),0.0);
//                        tmpOutWeights.append(allW);
//                        tempSumOutWeights.push_back(tmpSumWeights);
                        tempSumOutWeights.push_back(tmpSumWeights);
                    }
                    allW = std::accumulate(tempSumOutWeights[0].begin(),tempSumOutWeights[0].end(),0.0);
                    tmpOutWeights.append(allW);
                    ...
                    tempWeights.push_back(tmpOutWeights);
                }
                tmpAllWeightsArray.push_back(tempWeights);
            }
        }
...
/*
                        ...
                        if (weightMeshCheck == 1)
                        {
                            ...
                            for(i = 0; i < tempSumOutWeights[0].size(); i++ )
                            {

                                if (allUAnimCurveWeights[i][0][i].size() >= idx+1 and allVAnimCurveWeights[i][0][i].size() >= idx+1)
                                {
                                    tempSumOutWeights[0][i] = (tempSumOutWeights[0][i] *
                                                     allUAnimCurveWeights[i][0][i][idx] *
                                                     allVAnimCurveWeights[i][0][i][idx]);
                                }
                                else
                                {
                                    tempSumOutWeights[0][i] = tempSumOutWeights[0][i] * 1.0;
                                }
                            }
                            //sum of val array
                            allW = std::accumulate(tempSumOutWeights[0].begin(),tempSumOutWeights[0].end(),0.0);
                ...
                allOutWeightsArray.push_back(tempWeights);
            }

        float myfloat = fabs(allOutWeightsArray[0][0][0].length());
        MGlobal::displayInfo(MString()+myfloat);
//
...
//            if (success == 1)
//            {
//                std::vector < std::vector < MDoubleArray > > tmpAllSumWeightsArray;
//                for(j = 0; j < 4; j++ )
//                {
//                    std::vector < MDoubleArray > tempSumOutWeights;
...
//                                    tmp.append(1.0);
//                            }
//                            tempSumOutWeights.push_back(tmp);
//                        }
//                        tmpAllSumWeightsArray.push_back(tempSumOutWeights);
//                    }
//                    else
//                    {
//                        std::vector < std::vector < MDoubleArray > > tmpAllSumWeightsArray;
...
//                    }
//                }
//                allOutWeightsArray.push_back(tmpAllSumWeightsArray);
//            }
//            else if (success == 0)
//            {
//                // if nothing connected build dummy array to avoid future crashes
...
MStatus LHWeightDeformer::initialize() {
    ...
    //---child weight
    aWeights = tAttr.create("idxWeights", "iw", MFnNumericData::kDoubleArray);
