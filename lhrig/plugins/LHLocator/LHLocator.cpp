...
MObject LHLocator::aFaceIds;
MObject LHLocator::aGeo;
...
MObject LHLocator::oGeo;
//MObject oGeo;
MStatus LHLocator::compute( const MPlug& plug, MDataBlock& data )
{
        MStatus stat;
//        MObject tmpGeo = data.inputValue(aGeo).asMeshTransformed();
//        oGeo =tmpGeo;
...
//        tx = data.inputValue(localPositionX).asDouble();
//        ty = data.inputValue(localPositionY).asDouble();
//        tz = data.inputValue(localPositionZ).asDouble();
//        MDataHandle handle(data.inputValue(aFaceIds) );
//        faceIds = MFnDoubleArrayData(handle.data()).array();
//        MGlobal::displayInfo(MString("SOMETHING"));

//        cachePlugs = data.inputValue(aCachePlugs).asInt();
//        MGlobal::displayInfo(MString("SOMETHING")+faceIds.length());
//        if (faceIds.length() == 0 or cachePlugs == 0)
//        {
//            MDataHandle handle(data.inputValue(aFaceIds) );
//            faceIds = MFnDoubleArrayData(handle.data()).array();
//
//            drawFaces = MPlug( thisMObj, aDrawFaces ).asInt();
...
//            tx = MPlug( thisMObj, localPositionX ).asDouble();
//            ty = MPlug( thisMObj, localPositionY ).asDouble();
//            tz = MPlug( thisMObj, localPositionZ ).asDouble();
//            type = MPlug( thisMObj, aType ).asInt();
//            //---get size
//            dist = MPlug(thisMObj, aSize).asMDistance();
//            size = (float)dist.asCentimeters();
//            //---get orient
//            orient = MPlug(thisMObj, aOrient).asInt();
//
//
//        }
...
//        //get locator specific attrs
//        tz = MPlug( thisMObj, localPositionZ ).asFloat();
...
//                    MGlobal::displayInfo(MString("SOMETHING"));
        ...
        {
            //Make Sure to cache all of this out as it could be really slow
//            MObject dhGeo = data.inputValue(aGeo).asMeshTransformed;
            MStatus stat;




            oGeo = MPlug( thisMObj, aGeo ).asMObject();
            MFnMesh fnGeo(oGeo);


//            for (unsigned int i=0; i<faceIds.length();++i)
//            {
//                //get point ids in face id
//                //get points
//                //set points
...
////                                if (stat == MS::kSuccess)
////                                {
////                                    MGlobal::displayInfo(MString()+facePointIds[0]+MString(" ")+facePointIds[1]+MString(" ")+facePointIds[2]);
////                                }
////                        if (stat == MS::kFailure)
////                        {
////                            MGlobal::displayInfo(MString("Can't get polygon vertices"));
////                        }
//
...
////                            if (stat == MS::kFailure)
////                            {
////                                MGlobal::displayInfo(MString("Can't get point"));
////                            }
////                            MGlobal::displayInfo(MString("stuff"));
//                    if (i == 7 and j == 0)
//                    {
//                            MGlobal::displayInfo(MString()+pt.x+pt.y+pt.z);
...
//                   glColor4f(0.00f, 0.0f, 0.0f, 0.0f);
//                glEnable( GL_POLYGON_OFFSET_FILL);
//                glEnable( GL_POLYGON_OFFSET_LINE);
//                glEnable( GL_POLYGON_OFFSET_POINT);
...
//
//bool LHLocator::isTransparent( ) const
//{
//        return true;
//}
//
//bool LHLocator::drawLast() const
//{
//    return true;
//}
//
//bool LHLocator::isBounded() const
//{
//        return true;
//}
//
//MBoundingBox LHLocator::boundingBox() const
//{
//        MBoundingBox bbox;
//
//        unsigned int i;
...
//    addAttribute(aGeo);
    ...
    attributeAffects(aFaceIds, localPositionY);
