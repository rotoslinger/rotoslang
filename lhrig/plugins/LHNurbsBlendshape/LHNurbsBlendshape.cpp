...
//  	MPointArray finalTargetPositions:
  ...
  for (; !itGeo.isDone(); itGeo.next()) {
	...
	MPoint finalTargetPoint;
	MPoint finalAveragePoint;
	...
	for(i = 0; i < targetCount; i++ )
    {
    	...
    	finalTargetPoint += (targetPoints[i][itGeo.index()]- pt) * attrValArray[i];
    	finalAveragePoint+=averagePoint;
    	finalPatchPoint+=finalPatchPointTemp;

    }

	pt = pt + finalTargetPoint;
	pt =  pt - (finalAveragePoint - finalPatchPoint) * blendWeight;
