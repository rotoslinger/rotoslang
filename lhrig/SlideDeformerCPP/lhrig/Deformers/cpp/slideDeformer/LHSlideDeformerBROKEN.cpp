...
////////////////// get anim curves ////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////
MStatus LHSlideDeformer::getAnimCurves(MDataBlock &data,
                                       MObject &thisMObj,
...
//    float myfloat = fabs(rotationAmount);
//    MGlobal::displayInfo(MString()+myfloat);
    ...
    std::vector < std::vector <double> > allValsArray;
...
//        MGlobal::displayInfo(MString()+myfloat);
        std::vector <double> tmpValArray;

        for (j= 0; j < count; ++j)
        {

            arrayHandle.jumpToElement(j);
            tmpValArray.push_back(arrayHandle.inputValue().child( allValChildren[i] ).asDouble());
        }
...
//        if (newAllWeightsArray.size() > 0)
//            newAllWeightsArray.clear();
...
//              tmpAllWeightsArray = [];
                ...
                for(j = 0; j < 4; j++ )
                {
                    ...
                    {
                        numWeights = connectedArray.length();
...
//            float myfloat = fabs(allWeightsArray[0][2][0].length());
//            MGlobal::displayInfo(MString()+myfloat);
//            //newAllWeightsArray
//            MDataHandle dhInput = geomArrayHandle.inputValue();
//            MDataHandle dhWeightChild = dhInput.child( inputGeom );
...
//                        for(k = 0; k < numWeights; k++ )
//                        {
//                            if (allWeightsArray[i][j][k].length() > idx)
//                                tmp.append(allWeightsArray[i][j][k][idx]);
//                            else
//                                tmp.append(1.0);
//                        }
//                        tempWeights.push_back(tmp);
...
//                iter.next();
//            }
//            newAllWeightsArray.push_back(tmpAllWeightsArray);
//        }
...
//                    if (allWeightsArray.size() > numIndex)
//                    {
...
//                                }//    allWeightsArray = newAllWeightsArray;
...
//    MGlobal::displayInfo(MString()+myfloat);
    ...
    if (allUAnimCurveWeights.size() < numIndex or cacheWeightCurvesAmt == 0)
    {
        if (weightMeshCheck == 1)
        {
            ...
            for(i = 0; i < numIndex; i++ )
            {
                ...
                if (success == 1)
                {
                    for(j = 0; j < 4; j++ )
                    {
                        ...
                        {
                            //U
                            status = LHSlideDeformer::getAnimCurves(data, thisMObj,
            ...
            {
                //3-d arrays
                std::vector < std::vector < std::vector < double > > > tempU;
                std::vector < std::vector < MDoubleArray > > tempV;
...
//                                        MFnAnimCurve * wCurve(allUFnCurvesArray[i][j][k]);
                        ...
                        {
                            //2-d array
                            std::vector < MDoubleArray > tempVVals;
                            for(k = 0; k < allVFnCurvesArray[i][j].size(); k++ )
                            {//                          MDoubleArray tmp;

                                //1-d array
                                MDoubleArray tempVPointVals;
                                ...
                                for (; !iter.isDone();)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        MTime remapTime(remap);
                                        double weight = allVFnCurvesArray[i][j][k]->evaluate(remapTime) * allWeightsArray[i][j][k][idx] * allUAnimCurveWeights[i][j][k][idx];
                    ...
                    else if (iterGeoCount == 0)
                    {
                        ...
                        double dum = 0.0;
                        MDoubleArray dum1d;
                        std::vector < MDoubleArray > dum2d;
                        std::vector < std::vector < MDoubleArray > >  dum3d;
                        dum1d.append(dum);
                        dum2d.push_back(dum1d);
                        dum3d.push_back(dum2d);
                        allVAnimCurveWeights.push_back(dum3d);
                    }
                }
                else if (success == 0)
                {
    ...
    allWeightsArray = allVAnimCurveWeights;
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    {
        //Create intersector for closest point
    ...
    if (rotMatrix.size() < numIndex or cacheBaseAmt == 0)
    {
            ...
            if (success == 1)
            {
                ...
                if (iterGeoCount > 0)
                {
                    ...
                    for (; !iter.isDone();)
                    {
                        ...
                        MVector xBaseVector = xVecBase;
                        xBaseVector.normalize();

                        MVector yBaseVector = yVecBase;
                        yBaseVector.normalize();

                        MVector zBaseVector = xBaseVector ^ yBaseVector;
        ...
        ;
    //compose weights
    //if either weights or curve weights are uncached this will compose weights
    //check to make sure values
    ...
    for (; !MitGeo.isDone();)
    {
        ...
        for(i = 0; i < allValsArray.size(); i++ )
        {
            ...
            {
                tempVals.push_back(allValsArray[i][j] * allWeightsArray[mIndex][i][j][idx]);
            }
...
//        }//                tempVal = allValsArray[i][j];
        ...
        //                if (allUAnimCurveWeights[mIndex][i][j].size() >= idx+1 and allVAnimCurveWeights[mIndex][i][j].size() >= idx+1)
        //                {
...
MStatus LHSlideDeformer::initialize() {
    ...
    //////////////////////// VALUE ////////////////////////////////////////////////////
    aUValue = nAttr.create("uValue", "uv", MFnNumericData::kDouble);
    nAttr.setKeyable(true);
    ...
    //////////////////////// VALUE ////////////////////////////////////////////////////
    aVValue = nAttr.create("vValue", "vv", MFnNumericData::kDouble);
    nAttr.setKeyable(true);
    nAttr.setDefault(0.0);
    ...
    aNValue = nAttr.create("nValue", "nv", MFnNumericData::kDouble);
    nAttr.setKeyable(true);
    nAttr.setDefault(0.0);
    ...
    aRValue = nAttr.create("rValue", "rv", MFnNumericData::kDouble);
    nAttr.setKeyable(true);
    nAttr.setDefault(0.0);
